...
Pre-registered in the pre-registration record (not part of this release) (2026-09-13 23:55).  Until now every result used a quota of
...
def measure_router_overheads(n_frames=400):
    ...
    if runs:
        ...
        paths = {"nuScenes": sorted(_g.glob(str(_DS / "nuscenes/trainval/samples/CAM_FRONT/*.jpg")))[:200],
                 "KITTI": sorted(_g.glob(str(_DS / "kitti_tracking/training/image_02/0000/*.png")))[:200]}
        decoded = {}
