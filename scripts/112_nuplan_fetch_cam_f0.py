...
Pre-registered in the pre-registration record (not part of this release) (2026-09-14 14:44, "Task 5 pre-registration").  Nothing but metadata
...
OUT_ROOT = Path(str(_DS / "nuplan/sensor_blobs_cam_f0"))
CD_CAP = 400_000_000
IMG_CAP = 3_500_000_000
UA = "perception-value-release/1.0 (CAM_F0 window members only)"
URLS_FILE = ROOT / "configs" / "nuplan_camera_urls.txt"
METADATA_FILE = ROOT / "configs" / "nuplan_mini_sensor.txt"
NEEDED = WORK / "needed_cam_f0.csv.gz"         # the CAM_F0 images of the 34 benchmark logs, with scenario-window flags
...
def shard_urls() -> dict[int, tuple[str, str]]:
    ...
    if not URLS_FILE.exists():
        raise SystemExit(f"missing {URLS_FILE}: after logging in to the official nuPlan download page and accepting its "
                         ...
                         "'<shard index><TAB><url>' per line (see data/README.md)")
    urls = {}
