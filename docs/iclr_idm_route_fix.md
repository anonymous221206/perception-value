...
The per-state route geometry is in `results/raw/idm_route_diagnostics/route_geometry.csv`.
...
* Supplementary chain (03:50–04:01): 93 primary and 93 `--suffix _1thread` (measured overheads reused), then 94.
  The main chain had missed these two budget runs; they were rerun separately with the same checks.
The pre-fix values can be regenerated with `--no_route_fix`. The machine-readable before/after comparison is in
...
| perception filter untouched | CHEAP, FULL and reference track counts equal the pre-fix run on all states |
...
| rows that must not move | all non-IDM rows identical to the pre-fix outputs in 13 files: benchmark table and cells, router table, four budget tables (routers, nuPlan real, primary two-level, 1-thread two-level), both multi-fidelity tables, Task 5 cells, Task 7 table, both PDM-Closed raw files |
