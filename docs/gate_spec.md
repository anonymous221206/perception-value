...
|---|---|---|
| 1 | `ego_speed` | L103: `r = {"ego_speed": ego.dynamic_car_state.rear_axle_velocity_2d.x,` |
| 2 | `ego_accel` | L104: `"ego_accel": ego.dynamic_car_state.rear_axle_acceleration_2d.x,` |
| 3 | `n_cheap` | L105: `"n_cheap": int(kc.sum()), "n_cheap_fov": int(seen_m.sum())}` |
| 4 | `n_cheap_fov` | L105: `"n_cheap": int(kc.sum()), "n_cheap_fov": int(seen_m.sum())}` |
| 5 | `n_cheap_fov_bicycle` | L107: `r[f"n_cheap_fov_{c}"] = int((seen_m & (g["coarse"] == c)).sum())` |
| 6 | `n_cheap_fov_pedestrian` | L107: `r[f"n_cheap_fov_{c}"] = int((seen_m & (g["coarse"] == c)).sum())` |
| 7 | `n_cheap_fov_vehicle` | L107: `r[f"n_cheap_fov_{c}"] = int((seen_m & (g["coarse"] == c)).sum())` |
| 8 | `n_static_fov` | L108: `r["n_static_fov"] = int((seen & (g["coarse"] == "static")).sum())` |
| 9 | `min_range_fov` | L109: `r["min_range_fov"] = float(g["dist"][seen].min()) if seen.any() else FAR` |
| 10 | `min_gap_corridor` | L110: `r["min_gap_corridor"] = float(g["long_near"][mc].min()) if mc.any() else FAR` |
| 11 | `n_corridor_20` | L111: `r["n_corridor_20"] = int((mc & (g["long_near"] < 20)).sum())` |
| 12 | `n_corridor_40` | L112: `r["n_corridor_40"] = int((mc & (g["long_near"] < 40)).sum())` |
| 13 | `min_ttc_corridor` | L113: `r["min_ttc_corridor"] = float(min(g["ttc"][mc].min(), TTC_CAP)) if mc.any() else TTC_CAP` |
| 14 | `n_fov_band_30_40` | L114: `r["n_fov_band_30_40"] = int((seen_m & (g["dist"] >= 30) & (g["dist"] < 40)).sum())` |
| 15 | `crit_cheap_sum` | L115: `r["crit_cheap_sum"] = float(g["crit"][seen].sum())` |
| 16 | `crit_cheap_max` | L116: `r["crit_cheap_max"] = float(g["crit"][seen].max()) if seen.any() else 0.0` |
| 17 | `n_red_lights` | L117: `r["n_red_lights"] = int(sum(1 for t in sc.get_traffic_light_status_at_iteration(it)` |
| 18 | `d_n_cheap_fov` | L119: `r["d_n_cheap_fov"] = r["n_cheap_fov"] - (n_cheap_fov(filt, sc, it - 1) if it > 0 else r["n_cheap_fov"])` |
