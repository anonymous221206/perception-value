...
* The IDM part of the reading was already fixed once IDM finished, before PDM-Closed completed (the pre-registration record interim
  entry).
...
* **Camera 2–8 URLs follow the naming pattern of the official links.** Each was verified by HEAD and by its directory's log set.
