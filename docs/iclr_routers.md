...
Pre-registration: the pre-registration record (not part of this release), 2026-09-14 "Task 2 pre-registration", committed before any run.
...
* **Board incidents.**
  * The board rebooted twice during the Task 2 chain. The telemetry showed no memory or heat cause at
    either reboot; see the run log. Every result here comes from a run that completed.
